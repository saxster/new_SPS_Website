...
class NewsMiner:
    ...
    def fetch_signals(self, query=None, sector="General", limit=3):
        ...
        if not query:
            search_query = f"cybersecurity breach OR factory fire OR bank fraud India when:24h"
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sector", type=str, default="General", help="Industry vertical")
    ...
    miner = NewsMiner()
    results = miner.fetch_signals(query=args.query, sector=args.sector, limit=args.limit)
    ...
    if args.json:
        ...
        print(f"--- [MINER] Mission: {args.sector} | Query: {args.query or 'Default'} ---")
